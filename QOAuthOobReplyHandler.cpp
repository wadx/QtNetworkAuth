...
void QOAuthOobReplyHandler::networkReplyFinished(QNetworkReply* reply)
{
	...
	{
		qCWarning(NetworkAuthLogging, "%s", qPrintable(reply->errorString()));
		return;
	}
	if (reply->header(QNetworkRequest::ContentTypeHeader).isNull())
	{
		qCWarning(NetworkAuthLogging, "Empty Content-type header");
		return;
	}
	...
	if (data.isEmpty())
	{
		qCWarning(NetworkAuthLogging, "No received data");
		return;
	}
	...
	if (contentType.startsWith(QStringLiteral("text/html")) || contentType.startsWith(QStringLiteral("application/x-www-form-urlencoded")))
	{
		ret = parseResponse(data);
	}
	else if (contentType.startsWith(QStringLiteral("application/json")) || contentType.startsWith(QStringLiteral("text/javascript")))
	{
		const QJsonDocument document = QJsonDocument::fromJson(data);
		if (!document.isObject())
		{
			qCWarning(NetworkAuthLogging, "Received data is not a JSON object: %s", qPrintable(QString::fromUtf8(data)));
			return;
		}
		const QJsonObject object = document.object();
		if (object.isEmpty())
		{
			qCWarning(NetworkAuthLogging, "Received empty JSON object: %s", qPrintable(QString::fromUtf8(data)));
		}
		ret = object.toVariantMap();
	}
	else
	{
		qCWarning(NetworkAuthLogging, "Unknown Content-type: %s", qPrintable(contentType));
		return;
	}
